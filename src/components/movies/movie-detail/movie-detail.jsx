    ...
    <>
      <div className="row g-4">
        ...
        <div className="col-12 col-sm-8 col-md-8 col-lg-10">
          ...
          <div className="mb-2">
            <i className="fa fa-star text-warning"></i> {rating.toFixed(1)}
          </div>
          <div className="d-flex gap-2">
            {genres.map(({ id, name }) => (
              <span key={id} className="badge rounded-pill text-bg-light">
                {name}
              </span>
            ))}
          </div>
      ...
      {trailers.length > 0 && (
        <>
          ...
          <div className="d-flex gap-2 flex-wrap mb-3">
            {trailers.slice(0, 2).map((trailer, i) => (
              <ReactPlayer key={i} url={trailer} playing={false} />
            ))}
          </div>
        </>
      )}
      {recs.length > 0 && <MoviesList movies={recs} title="Recommendations" />}
    </>
  );
}
